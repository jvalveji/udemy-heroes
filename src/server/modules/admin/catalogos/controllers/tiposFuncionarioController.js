// Definición JavaScript para el controlador catálogo tipos funcionarios v3.0.0
// Proyecto: Arca - MEAN
...
module.exports = {
		...
		catalogoModel.find(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de Tipos de Funcionario.',
					error: err
				});
			}
			if (!catalogo) {
	...
	/**
	 * @description Función encargada de obtener los datos de un tipo de funcionarios por su id
	 */
	show: function (req, res) {
		...
		catalogoModel.findOne(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de Tipos de funcionarios.',
					error: err
				});
			}
			if (!catalogo) {
