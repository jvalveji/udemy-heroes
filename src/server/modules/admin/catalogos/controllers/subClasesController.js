// Definición JavaScript para el controlador catálogo sub clases siges v3.0.0
// Proyecto: Arca - MEAN
...
const catalogoModel = require('./../models/subClasesArticulosModel');
...
/*
 * subClasesController.js
 * @description :: Server-side logic for managing catalogos.
 */
module.exports = {
	/**
	 * @description Función encargada de obtener el catálogo de
	 * sub-clases de artículos que provienen de la collection creada con información de SIGES.
	 */
		...
		catalogoModel.find(_where, function (err, catalogos) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener las sub-clases de articulos.',
					error: err
				});
			}
	...
	/**
	 * @description Función encargada de obtener el catálogo de
	 * sub-clases de artículos por ID que provienen de la collection creada con información de SIGES.
	 */
	showByClase: function (req, res) {
		...
		catalogoModel.find(_where, function (err, catalogos) {
			...
			var clase_id = mongoose.Types.ObjectId(req.params.id);
			var subClasesObtenidas = [];
			async.eachSeries(catalogos, function (subClase, callbackInicial) {
				if (subClase.clase_id.equals(clase_id)) {
					let subClaseEncontrada = {
						'_id': subClase._id,
						'clase_id': subClase.clase_id,
						'descripcion': subClase.descripcion,
						'idSIGES': subClase.idSIGES
					};
					subClasesObtenidas.push(subClaseEncontrada);
				}
				callbackInicial();
			}, function (err) {
				...
				if (err) {
					// Se crea un objeto respuesta y los datos se encapsulan en este
					const respuesta = new HttpResponse(false, 'Error tratando de obtener la subclase.', null);
					// Se obtiene el JSON del mensaje y se envia (500 = Internal Server Error)
					return res.status(500).send(respuesta.getJSON());
				}

				// Se crea un objeto respuesta y los datos se encapsulan en este
				const respuesta = new HttpResponse(true, null, subClasesObtenidas);
				// Se obtiene el JSON del mensaje y se envia (200 = OK)
				return res.status(200).send(respuesta.getJSON());
			});
		}).sort({
