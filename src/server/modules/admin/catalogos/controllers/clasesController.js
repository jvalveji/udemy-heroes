// Definición JavaScript para el controlador catálogo clases v3.0.0
// Proyecto: Arca - MEAN
...
const catalogoModel = require('./../models/clasesArticulosModel');
...
module.exports = {
	 ...
	 * clases de artículos que provienen de la collection creada con información de SIGES.
	 */
		...
		catalogoModel.find(_where, function (err, catalogos) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener las clases de articulos.',
					error: err
				});
			}
	...
	/**
	 * @description Función encargada de obtener los datos de un articulo por su id
	 */
	show: function (req, res) {
		...
		catalogoModel.findOne(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de articulos.',
					error: err
				});
			}
			if (!catalogo) {
