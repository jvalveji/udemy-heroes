// Definición JavaScript para el controlador catálogo especialidades v3.0.0
// Proyecto: Arca - MEAN
...
const catalogoModel = require('./../models/especialidadesModel');
...
module.exports = {
		...
		catalogoModel.find(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de Especialidades.',
					error: err
				});
			}
			if (!catalogo) {
	 ...
	 * del catálogo de especialidades aquellos que pertenezcan al servicio específico
	 */
	showByServicio: function (req, res) {
		// Obtien el id del grupo y lo almacena en una variable local
		const idServicio = req.params.idServicio;
		// Variable que establece el filtro de la consulta a la base de datos (WHERE)
		const _where = {
			'servicio_id._id': mongoose.Types.ObjectId(idServicio)
		};
		// Ejecuta una función de agregación para obtener los datos por el filtro
		catalogoModel.find(_where, function (err, items) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de especialidades basado en el servicio indicado.',
					error: err
				});
			}
			// Se crea un objeto respuesta y los datos se encapsulan en este
	...
	update: function (req, res) {
		...
		catalogoModel.findOne(_where, function (err, data) {
			...
			data.set({
				...
				idServicioSIAH: req.body.idServicioSIAH,
