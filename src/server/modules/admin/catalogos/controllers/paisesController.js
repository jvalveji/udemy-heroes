// Definición JavaScript para el controlador catálogo paises v3.0.0
// Proyecto: Arca - MEAN
...
const catalogoModel = require('./../models/paisesModel');
...
module.exports = {
		...
		catalogoModel.find(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de Paises.',
					error: err
				});
			}
			if (!catalogo) {
	...
	/**
	 * @description Función encargada de obtener los datos de un país por su id
	 */
	show: function (req, res) {
		...
		catalogoModel.findOne(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de país.',
					error: err
				});
			}
			if (!catalogo) {
	...
	update: function (req, res) {
		...
		catalogoModel.findOne(_where, function (err, data) {
			...
			data.set({
				...
				isoNumerico: req.body.isoNumerico,
				descripcion: req.body.descripcion,
				isoAlfa3: req.body.isoAlfa3,
