// Definición JavaScript para el controlador catálogo grupo rh v3.0.0
// Proyecto: Arca - MEAN
...
module.exports = {
		...
		catalogoModel.find(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de nutrición.',
					error: err
				});
			}
			if (!catalogo) {
	...
	/**
	 * @description Función encargada de obtener los datos de un grupo RH por su id
	 */
	show: function (req, res) {
		...
		catalogoModel.findOne(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de grupos RH.',
					error: err
				});
			}
			if (!catalogo) {
