// Definición JavaScript para el controlador catálogo unidades de medida v3.0.0
// Proyecto: Arca - MEAN
...
const catalogoModel = require('./../models/unidadesMedidaModel');
...
module.exports = {
		...
		catalogoModel.find(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de unidades de medida.',
					error: err
				});
			}
			if (!catalogo) {
	...
	/**
   * @description Función encargada de obtener el catálogo de las unidades de medida
   */
