// Definición JavaScript para el controlador catálogo de etls v3.0.0
// Proyecto: Arca - MEAN
...
const catalogoModel = require('./../models/etlsModel');
...
module.exports = {
	/**
	 * @description Función encargada de obtener el catálogo de etls
	 */
		...
		catalogoModel.find(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el catálogo de etls.',
					error: err
				});
			}
			if (!catalogo) {
	...
	/**
	 * @description Función encargada de obtener el por nombre desde el catálogo de etls
	 */
	showByNombre: function (req, res) {
		// Variable que establece el filtro de la consulta a la base de datos (WHERE)
		const _where = {
			'idEtl': req.params.name
		};
		// Busca la información del catalogo
		catalogoModel.find(_where, function (err, catalogo) {
			if (err) {
				return res.status(500).json({
					message: 'No se pudo obtener el item desde el catálogo de etls.',
					error: err
				});
			}
			if (!catalogo) {
