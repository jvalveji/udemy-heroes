...
const grupoMaterial = require('../models/grupoMaterialModel');
...
module.exports = {
    ...
    getGrupoMaterial: (req, res) => {
        //Buscar los Materiales
        var query = grupoMaterial.find();
        ...
        query.sort('-_id').exec((err, material) => {
            ...
            if (err) {
                return res.status(500).send({
                    status: 'error',
                    menssage: 'Error al devolver los grupos de materiales!!!!!'
                });
            }
            ...
            if (!material) {
                return res.status(404).send({
                    status: 'error',
                    menssage: 'No hay grupos de materiales para mostrar!!!'
                });
            }
