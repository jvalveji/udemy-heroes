...
const catalogoModel = require('../models/familiasArticulosModel');

const grupoModel = require('../models/grupoArticulosModel');
...
module.exports = {
    ...
    showByGrupo: function(req, res) {
        ...
        const id_grupov = req.params.id_grupo;
        // Variable que establece el filtro de la consulta a la base de datos (WHERE)
        const _where = {
            id_grupo: mongoose.Types.ObjectId(id_grupov)
        };


        // verificar la exitencia del id
        if (!id_grupov || id_grupov == null) {
            return res.status(404).send({
                status: 'error',
                menssage: 'No existe el grupo.'
            });
        }
        ...
        catalogoModel.find(_where)
            ...
            .exec((err, items) => {
                if (err) {
                    return res.status(500).json({
                        message: 'No se pudo obtener el catálogo de familias.',
                        error: err
                    });
                }
                // Se crea un objeto respuesta y los datos se encapsulan en este
