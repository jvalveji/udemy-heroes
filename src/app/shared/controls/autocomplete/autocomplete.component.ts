...
export class AutocompleteComponent implements OnInit {
	...
	set Disable(_disable: boolean) {
		...
		if (_disable) { this.filtro.disable(); } else { this.filtro.enable(); }
	}
	...
	*/
	...
	private FiltrarItems(filtro: string): any {
		...
		this.itemSeleccionado = (filtro && this.source) ? this.source.filter(s => s.descripcion.toLowerCase().indexOf(filtro.toLowerCase()) !== -1)
			: this.source;
	...
	public ngOnInit() {
		...
		if (typeof this.source !== 'undefined' && this.source !== null && this.source.length !== 0) {
			...
			VALOR_DEFAULT = this.source.filter(s => (s.hasOwnProperty('id')) ? s.id : s._id === this.defaultValue);
			// Obtiene la descripción del valor default
			if (VALOR_DEFAULT.length > 0) { VALOR_DEFAULT = VALOR_DEFAULT[0].descripcion };
		}
		// Establece el filtro
