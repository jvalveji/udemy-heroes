...
// Definición typescript para el servicio ProfesionalesService v3.0.0
// Proyecto: Arca - MEAN
...
/** Servicio con las funcionalidades para operar mediante el protocolo HTTP para profesionales del catálogo general */
@Injectable()
export class ProfesionalesService implements ICRUD {
	/** Url del servicio api a consumir por el servicio */
	private apiUrl = environment.urlApi.main + '/admin/catalogos/profesionales'; // URL web API
	/**
	...
	public Update(data: any): Promise<IHttpResponse> {
		...
		return new Promise((resolve, reject) => {
			this.http
				.put(this.apiUrl, { items: data }).pipe(
					map((res: IHttpResponse) => res))
				.subscribe(res => {
					resolve(res);
				}, (err) => {
