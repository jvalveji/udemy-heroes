// Definición typescript para el componente TiposMonedaEditarComponent v2.0.0
// Proyecto: Arca - MEAN
...
/**
 * Componente destinado al despligue y manejo del catálogo de monedas
 */
@Component({
	selector: 'arca-tipos-moneda-editar',
	templateUrl: './tipos-moneda-editar.component.html',
	styleUrls: ['./tipos-moneda-editar.component.scss']
})
export class TiposMonedaEditarComponent implements OnInit {
	...
	constructor(private _location: Location,
		private catalogoService: TiposMonedaService,
	...
	public AgregarItem(): void {
		this.esEdicion = true;
		/**Validacion para comprobar que todos los campo fueron completados */
		if (
			this.item.indicador === null ||
			this.item.indicador === undefined ||
			this.item.indicador === '' ||
			this.item.descripcion === null ||
			this.item.descripcion === undefined ||
			this.item.descripcion === '' ||
			this.item.simbolo === null ||
			this.item.simbolo === undefined ||
			this.item.simbolo === ''
		) {
	...
	/**
	 * Método encargado de inicializar los campos del item
	 */
	public LimpiarItem() {
		this.item = null;
		this.item = {
	...
	/**
	 * Método en cargado de obtener el catalogo de monedas disponible.
	 */
