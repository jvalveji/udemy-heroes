// Definición typescript para el componente CantonesEditarComponent v4.0.0
// Proyecto: Arca - MEAN
...
@Activate(['adm_cat_cantones'])
/**
 * Componente destinado al despligue y manejo del catálogo de cantones
 */
@Component({
	selector: 'arca-cantones',
	templateUrl: './cantones-editar.component.html',
	styleUrls: ['./cantones-editar.component.scss']
})
export class CantonesEditarComponent implements OnInit {
	/**
	 * Variable para mostrar/ocultar la barra de progreso
	 */
	public esCargando = false;
	/**
	 * Variable que almacena los datos obtenidos del catálogo de cantones por provincia
	 */
	public catalogo: any = [];
	/**
	 ...
	 * @param catalogoService Variable que representa a los servicios de cantones del core base
	...
	constructor(private _location: Location,
		private catalogoService: CantonesService,
	...
	public ObtenerCantonByProvincia(data: any): void {
		...
		this.esCargando = true;
		// Se llama a la función del servicio que envia los datos al server
		this.catalogoService.ShowCantonByProvincia(data).then((res) => {
			// Oculta la barra de progreso una vez obtenida la respuesta
			this.esCargando = false;
			// Recibe la respuesta
			if (res.exito) {
