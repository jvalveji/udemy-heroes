...
export class MaterialesServiciosAccesoEditarComponent implements OnInit {
	...
	*/
	...
	/**
	 * Variable que almacena un elemento temporal
	 */
	...
	agregarMaterial() {
		if (!this.ExisteMaterial()) {
			...
			this.msgBox.open(
				"WARNING",
				"Advertencia",
				"No se pueden ingresar codigos duplicados"
			);
		}
	}
	 ...
	 */
	public eliminarMaterial(mat: any) {
		for (let i = 0; i < this.mat_sel.length; i++) {
			if (this.mat_sel[i]._id === mat._id) {
				cloneDeep(this.mat_sel.splice(i, 1));
				i--;
			}
		}
		console.log(this.item);
