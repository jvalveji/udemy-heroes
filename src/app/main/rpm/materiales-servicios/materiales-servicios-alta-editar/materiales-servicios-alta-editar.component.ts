...
export class MaterialesServiciosAltaEditarComponent implements OnInit {
	...
	*/
    ...
    private ObtenerCatalogoFamArt() {
        // Inicia la barra de progreso
        //this.esCargando = true;
        // Se llama a la función del servicio que envia los datos al server

        const filter2 = this.frmAltaMaterial.get('artMatServ')?.value;

        this.materialesServiciosAltaEditar.ShowFamByBrupo(filter2).then(
            (res) => {
                // Oculta la barra de progreso una vez obtenida la respuesta
                // this.esCargando = false;
                // Recibe la respuesta
    ...
    ngOnInit(): void {
        ...
        // Carga el catálogo de grupos de Articulos
        this.ObtenerCatalogoArticulo();
        //cuando cambie el grupo de articulo
        // this.ObtenerCatalogoFamArt(this.frmAltaMaterial.get('artMatServ')?.value);
        //this.ObtenerCatalogoFamArt('60d6483322ec2d4c34ba13a9');

        // this.frmAltaMaterial.get('artMatServ')?.valueChanges.pipe(
        //     tap((_) => {
        //         this.frmAltaMaterial.get('famMatServ')?.reset('');
        //         //this.cargando = true;

        //         this.ObtenerCatalogoFamArt('60d6483322ec2d4c34ba13a9');

        //         console.log('pasa por tap');
        //     })
        // );
        console.log('oninit');

        //this.frmAltaMaterial.get('artMatServ')?.value
    }
}
